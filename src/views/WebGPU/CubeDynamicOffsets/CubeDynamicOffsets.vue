 ...
 * @LastEditTime: 2023-06-29 23:53:54
...
const initPipeline = async (
    ...
    dynamicBindGroupLayout: GPUBindGroupLayout,
} => {
...
const creatBuffer = (device: GPUDevice, pipeline: GPURenderPipeline, dynamicBindGroupLayout: GPUBindGroupLayout, size: {width:number, height:number}) => {
...
const draw = (
    ...
    // set uniformGroup
    passEncoder.setBindGroup(0, bufferObj.uniformGroup, offset, 0, 1)
    // 3 vertex form a triangle
    passEncoder.draw(cubeVertex.length / 3)
    passEncoder.setBindGroup(0, bufferObj.uniformGroup, offset, 1, 1)
    passEncoder.draw(cubeVertex.length / 3)
    passEncoder.end()
...
const init = async () => {
    ...
    const {pipeline,dynamicBindGroupLayout} = await initPipeline(device, format)
    const bufferObj = creatBuffer(device, pipeline, dynamicBindGroupLayout, canvasSize)
